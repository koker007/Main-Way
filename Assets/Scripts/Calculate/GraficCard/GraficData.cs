...
public class GraficData : MonoBehaviour
{
    ...
    static int LastPerlin2DArrayID = 0;
    ...
    static public Perlin2DArray[] dataPerlin2DArray = new Perlin2DArray[IDMax];
    ...
    public class Perlin2DArray
    {
        ...
        public Perlin2DArray(float scaleX, float scaleY, float scaleZ, float frequency, float offsetX, float offsetY, float offsetZ, int octaves, int repeatX, int repeatY, float regionX, float regionY, int ArrayTextures)
        {
            result = new float[32,32, ArrayTextures];

            id = LastPerlin2DArrayID;
            LastPerlin2DArrayID++;

            //Если id стало больше 1000 то заполняем данные заного
            if (LastPerlin2DArrayID >= IDMax)
                LastPerlin2DArrayID = 0;
    ...
    public class Perlin3DRules
    {
        ...
        public Perlin3DRules(BiomeData.GenRule[] genRules, int repeatX, int repeatY, int repeatZ, float regionX, float regionY, float regionZ)
        {
    ...
    public class BlockVoxel
    {
        ...
        const int verticesCount = (16 * 16 * 16 + 16 * 16) * 4 * 3; //4 вершины на 3 лицевые сторонны всех вокселей + 3 закрывающие сторонны с 4 вершинами размером 16на16 
    ...
    public class ChankWorldData {
        //Данные готовы к расчету или нет
        ...
        //Карта представляет из себя участок размером 10 на 10 (то есть верхняя часть биома)
        public int[,] mapOfHeight;
        public int[,] mapOfTemperature;

        //Инициализировать данные к расчету
